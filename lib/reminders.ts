  ...
  for (const b of upcoming) {
    ...
    if (in24hWindow) {
      ...
      if (!b.sms_24h_sent && hasContact) {
        const message = `SEDO: Hi ${b.name}! Reminder: your Discovery Call is tomorrow, ${formatDateLabel(date)} at ${formatTimeLabel(timeHHMM)} (GMT+8).${zoomLink ? ` Zoom: ${zoomLink}` : ''}`;
    ...
    if (in1hWindow) {
      ...
      if (!b.sms_1h_sent && hasContact) {
        const message = `SEDO: Hi ${b.name}! Your Discovery Call starts in 1 hour, ${formatTimeLabel(timeHHMM)} (GMT+8).${zoomLink ? ` Zoom: ${zoomLink}` : ''}`;
        const result = await sendSms(b.contact!, message);
        if (result.sent) {
