...
// Exported version for use in API routes
...
function seedUsersIfEmpty() {
  ...
  for (const m of allModules) insertPerm.run(ownerId, m);
  ...
  for (const s of staff) {
    ...
    for (const m of s.modules) insertPerm.run(uid, m);
  }
}
...
function seedIfEmpty() {
  ...
  const insertProduct = db.prepare(
    'INSERT INTO products (sku,name,category,cogs,srp,reorder_point) VALUES (@sku,@name,@category,@cogs,@srp,@reorder_point)'
  );
  ...
  txn(() => {
    for (let i = 0; i < products.length; i++) {
      const info = insertProduct.run(products[i]);
      insertInventory.run(Number(info.lastInsertRowid), startQty[i], fmt(now));
    }
  });
  ...
  const fastMoverIdxs = [0, 5, 10];
  const slowMoverIdxs = [4, 11];

  txn(() => {
    for (let day = 30; day >= 1; day--) {
      const date = daysAgo(day);
      for (const idx of fastMoverIdxs) {
      ...
      for (let i = 0; i < pids.length; i++) {
        if (fastMoverIdxs.includes(i) || slowMoverIdxs.includes(i)) continue;
      ...
      for (const idx of slowMoverIdxs) {
        if (day % 7 === 0) insertMove.run(pids[idx], 'OUT', 1, 'Sale', date);
      }
    }
    const today = fmt(now);
  ...
  const insertPO = db.prepare(
    'INSERT INTO purchase_orders (po_number,supplier,total_amount,status,ordered_at,received_at,notes) VALUES (?,?,?,?,?,?,?)'
  );
  ...
  txn(() => {
    ...
    insertPOItem.run(Number(po1.lastInsertRowid), pids[0], 30, 450);
    insertPOItem.run(Number(po1.lastInsertRowid), pids[1], 10, 1200);

    const po2 = insertPO.run('PO-20260617-001','Fashion Hub Manila',9600,'pending',daysAgo(7),null,'Awaiting delivery');
    insertPOItem.run(Number(po2.lastInsertRowid), pids[5], 30, 120);
    insertPOItem.run(Number(po2.lastInsertRowid), pids[6], 20, 200);
    insertPOItem.run(Number(po2.lastInsertRowid), pids[7], 10, 280);

    const po3 = insertPO.run('PO-20260601-001','HomeKing Wholesale',5800,'cancelled',daysAgo(23),null,'Supplier unavailable');
    insertPOItem.run(Number(po3.lastInsertRowid), pids[11], 20, 220);
    insertPOItem.run(Number(po3.lastInsertRowid), pids[12], 10, 310);
  });
  ...
  txn(() => {
    insertResearch.run('LED Strip Lights RGB',      1,'https://google.com/search?q=led+strip+lights',     120,349,'HomeLux PH',    'Maria Santos', 'Done');
    insertResearch.run('Magnetic Phone Case',        0,'https://google.com/search?q=magnetic+phone+case',  80,249,'GadgetZone PH', 'Juan dela Cruz','For Ads Testing');
    insertResearch.run('Posture Corrector Belt',     1,'https://google.com/search?q=posture+corrector',   150,450,'HealthFirst PH','Ana Reyes',     'For FB Page');
    insertResearch.run('Reusable Silicone Bag Set',  0,null,                                               95,280,null,            null,            'For Research');
    insertResearch.run('Wireless Charging Pad 15W',  0,'https://google.com/search?q=wireless+charger+15w',200,599,'TechDeals PH', 'Carlo Mendoza', 'For Ads Testing');
  });
}
