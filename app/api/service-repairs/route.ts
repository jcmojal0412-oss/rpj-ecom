...
export async function POST(req: NextRequest) {
  try {
    const db = getDb();
    const {
      repair_date, repair_details, unit_model, technician_name, cs_payment, cogs,
    ...
    const info = db.prepare(`
      INSERT INTO service_repairs
        (repair_date, repair_details, unit_model, technician_name, cs_payment, cogs, labor_amount,
         bns_share, gerald_share, dp, status, paid_to_tech, tech_paid_date, order_no, receipt_no)
      VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
    `).run(
      repair_date, repair_details ?? null, unit_model ?? null, technician_name?.trim() || null,
