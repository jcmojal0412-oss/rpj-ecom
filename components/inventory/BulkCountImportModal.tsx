        ...
        {stage === 'idle' || stage === 'error' ? (
          ...
          <div className="space-y-3">
            ...
            <div className="flex justify-between items-center pt-1">
              ...
              <button onClick={confirmImport} disabled={preview.change_count === 0} className="btn-primary text-sm min-h-[44px] sm:min-h-0 disabled:opacity-40">
          ...
          <div className="space-y-3">
            ...
            <button onClick={startOver} className="py-2.5 sm:py-0 text-xs text-blue-600 hover:text-blue-800 font-medium">← Mag-upload ng isa pang file</button>
          </div>
        ) : null}
      </div>

      <div className="flex justify-end pt-2 border-t border-gray-100">
        <button onClick={onClose} className="btn-secondary text-sm min-h-[44px] sm:min-h-0">{stage === 'done' ? 'Close' : 'Cancel'}</button>
      </div>
    </div>
  );
}
