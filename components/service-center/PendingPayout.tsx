...
import { toLocalISO, weekStart, weekLabel, isCurrentWeek, payoutDate } from './weekUtils';
      ...
      for (const r of items) {
        await fetch(`/api/service-repairs/${r.id}`, {
          ...
          body: JSON.stringify({
            ...
            technician_name: r.technician_name, order_no: r.order_no, receipt_no: r.receipt_no,
            ...
            <div key={key} className={`border rounded-xl p-4 ${current ? 'border-amber-100 bg-amber-50/40' : 'border-red-200 bg-red-50/40'}`}>
              <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{items.length} job{items.length > 1 ? 's' : ''} · {formatCurrency(subtotal)}</p>
                  <p className="text-xs text-gray-500 mt-0.5">Payout Date: {formatDate(toLocalISO(payDate))}</p>
                  <span className={`inline-block mt-1 text-[11px] font-semibold px-2 py-0.5 rounded-full ${current ? 'bg-amber-100 text-amber-700' : 'bg-red-100 text-red-700'}`}>
                    {current ? 'For Next Monday' : 'Unpaid'}
                  </span>
                </div>
                <button
